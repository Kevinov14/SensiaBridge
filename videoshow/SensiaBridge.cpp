...
namespace proyectoSensiaBridge {
            ...
            try {
                ...
                if (priv->init_error != nullptr) {
                    ...
                    try{
                        ...
                        Saca_FOTOS(priv);
                        //copiarArvBufferABuffer(*buffer, buff);
   ...
   cv::Mat aravis_to_opencv(ArvBuffer *buffer) {
        assert(arv_buffer_get_payload_type(buffer) == ARV_BUFFER_PAYLOAD_TYPE_IMAGE);
        if (buffer == nullptr) {
            std::cerr << "Buffer vacío de Aravis" << std::endl;
            return cv::Mat();
        }

        // Obtener datos de la imagen desde el buffer de Aravis
        size_t buffer_size;
        //printf("Numero de partes: %d\n",  parts);
        
        const void *data = arv_buffer_get_data(buffer, &buffer_size);
       
        //printf("Cadena: %s\n", data);
        if (data == nullptr) {
            std::cerr << "Error al obtener datos del buffer de Aravis" << std::endl;
            return cv::Mat();
        }
        ...
        //uint16_t* raw_data = reinterpret_cast< uint16_t*>(arv_buffer_get_data(buffer, nullptr));

        //printf("Formato pixel: %d\n",pixel_format);
        // Crear un cv::Mat según el formato de la imagen
        cv::Mat img;
        //int width = 640; // Ejemplo de ancho
        //int height = 480; // Ejemplo de alto

        if (pixel_format == ARV_PIXEL_FORMAT_MONO_16) {
            //printf("Formato: %d\n", pixel_format);
            // Si es imagen en escala de grises (8 bits)
            img = cv::Mat(height, width, CV_16U, (unsigned char *)data);
            double minVal, maxVal;
            minMaxLoc(img, &minVal, &maxVal);
            img.convertTo(img, CV_8U, 255.0 / (maxVal - minVal), -minVal * 255.0 / (maxVal - minVal));
            //cv::cvtColor(img, img, cv::COLOR_GRAY2BGR);
            //img = cv::Mat(height, width, CV_16U, (void*)data);
            //img.convertTo(img_normalizada, CV_8UC1, 255.0/ 65535.0);
        } else if (pixel_format == ARV_PIXEL_FORMAT_MONO_14) {
            // Si es imagen RGB (24 bits)
            
            img = cv::Mat(height, width, CV_8UC1, (void*)data);
            
            
           // cv::cvtColor(img, img, cv::COLOR_RGB2BGR);  // Convertir de RGB a BGR para OpenCV
        } else {
            std::cerr << "Formato de píxel no soportado por OpenCV" << std::endl;
            return cv::Mat();
        }
    ...
    int Saca_FOTOS (ArvCameraPrivate *cam)
    {
        ...
        int height;
        int width;
        // Ruta donde quieres guardar el video
        std::string ruta = "C:/CODE/videoaravis-main/imgs/output_video.avi";
        cv::Mat img;
        //cv::Mat img8;
        ...
        std::string output_path = "C:/CODE/videoaravis-main/imgs/output_image.jpg";  // Ruta de salida (cambiar a la ruta deseada)
        if (cv::imwrite(output_path, img)) {
            std::cout << "Imagen guardada correctamente en " << output_path << std::endl;
        } else {
